      ...
      <section className="px-4 py-12 bg-white dark:bg-gray-900 transition duration-300">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className=" text-md mb-6 leading-relaxed">
            ...
            Once again, our city will be filled with magnificent sailing ships
            from all over the world. There will be adventurous experiences, open
            ships, exciting activities, concerts and experiences for the whole
            family – and it's all free!
          </p>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {news.map((article, index) => (
            <div
              key={index}
              className="bg-white shadow-sm rounded-xl p-4 hover:shadow-lg transition-shadow duration-300"
            >
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 gap-4">
          {schedule.map((item, index) => (
            <div
              ...
              className="bg-white shadow-sm rounded-xl p-4 hover:shadow-lg transition-shadow duration-300"
            >
              <p className="font-semibold table-header">{item.time}</p>
              <p className="table-header">{item.event}</p>
            </div>
          ))}
        </div>
      </section>
